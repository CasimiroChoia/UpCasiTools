        ...
        var numpcPontos = 0, numEuPontos = 0        //PONTOS INICIAIS DE CADA JOGADOR
    ...
    return `${new Date().getFullYear()}-${new Date().getMonth()+1}-${new Date().getDate()}-${horas}:${minutos}`
        ...
        function game() {
            ...
            euMeter = document.getElementById("euMeter")
            pcMeter = document.getElementById("pcMeter")

            euMeter.setAttribute("value", `${numEuPontos}`)
            pcMeter.setAttribute("value", `${numpcPontos}`)
            // euMeter.style.background = "rgb(238, 211, 59)"
            // pcMeter.style.background = "rgb(238, 211, 59)"
